...
def get_tesla_accel_limits(CP, current_speed):
    a_min = 0.
    a_max = interp(current_speed, ACCEL_LOOKUP_BP,
                   ACCEL_MAX_LOOKUP_V[ACCEL_PROFILE])
    if CP.carFingerprint == CAR.PREAP_MODELS and not HAS_IBOOSTER_ECU:
        ...
        a_max = interp(current_speed, ACCEL_LOOKUP_BP,
                       ACCEL_AP_MAX_LOOKUP_V[ACCEL_PROFILE])
        a_min = interp(current_speed, ACCEL_LOOKUP_BP, ACCEL_MIN_LOOKUP_V)
    return a_min, a_max
...
class CarInterface(CarInterfaceBase):
    ...
    def get_params(candidate, fingerprint=gen_empty_fingerprint(), car_fw=None):
        ...
        if candidate in(CAR.AP2_MODELS,CAR.AP2_MODELX):
            ...
            if (CAN_AP_POWERTRAIN[candidate] in fingerprint.keys()) and (0x2bf in fingerprint[CAN_AP_POWERTRAIN[candidate]].keys()):
              print("tesla_powertrain debug")
            #     ret.openpilotLongitudinalControl = True
            #     ret.safetyConfigs = [
            #         get_safety_config(
            #             car.CarParams.SafetyModel.tesla, safetyParam | Panda.FLAG_TESLA_LONG_CONTROL),
            #         get_safety_config(car.CarParams.SafetyModel.tesla, safetyParam |
            #                           Panda.FLAG_TESLA_LONG_CONTROL | Panda.FLAG_TESLA_POWERTRAIN),
            #     ]
            # else:
            ret.openpilotLongitudinalControl = False
            ret.safetyConfigs = [get_safety_config(
                    car.CarParams.SafetyModel.tesla, safetyParam)]
        else:
            ret.safetyConfigs = [get_safety_config(
                car.CarParams.SafetyModel.tesla, safetyParam)]
        if candidate == CAR.PREAP_MODELS:
            ...
            # since we don't use the PID, this means a jerk in acceleration by x m/s^3
    ...
    def update(self, c, can_strings):
        ...
        if self.CP.carFingerprint == CAR.PREAP_MODELS:
            ...
            ret.canErrorId = max(self.cp.error_address,
                                 self.cp_cam.error_address)
